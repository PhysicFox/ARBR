...
String a="jdbc:mysql://localhost/sportmanage?characterEncoding=utf-8&serverTimezone=GMT%2B8";//输入格式为汉字
String b="root";
String c="12345678";
...
<form action="dormitory_add_point.jsp" method="post">
<table border=1>
   <tr>
       <th>宿舍号</th>
       <th>床号</th>
   ...
   <tr>      
       ...
       <td><input type="text" name="bed" value=-1></td>
       <td><input type="text" name="class" value=-1></td>
...
<form action="dormitory_manage.jsp" method="post">
...
try{
	con=DriverManager.getConnection(a,b,c);	
	sql=con.createStatement();	
	sess_rs=sql.executeQuery("select * from dormitory where 扣分部门='"+identity+"' and 有效=1"); 
	
    if(fromda==null){
    	for(int i=0;i<12;i++){
			sess_rs=sql.executeQuery("select * from "+table[i]+" where 扣分部门='"+identity+"' and 有效=1");
 			int tablenum=i;  
			  while(sess_rs.next()){//下一条存放数据库的类的信息		
				  out.print("<tr>");        	
		        	out.print("<td>"+sess_rs.getString("宿舍号")+"</td>");//td为换列
		  	    	out.print("<td>"+sess_rs.getString("床号")+"</td>");
		  	    	out.print("<td>"+sess_rs.getString("班级")+"</td>");
			     	out.print("<td>"+sess_rs.getString("扣分日期")+"</td>");
		  		    out.print("<td>"+sess_rs.getString("扣分部门")+"</td>");
		     		out.print("<td>"+sess_rs.getString("扣分")+"</td>");
		     		out.print("<td>"+sess_rs.getString("扣分原因")+"</td>");

			  	    int ida=sess_rs.getInt("id");
		  		    out.print("<td>"+"<a href=delete_point_dormitory.jsp?id="+ida+"&tablen="+tablenum+">删除</a>"+"</td>");//跳转页面错误
		  	    	out.print("</tr>");	  
			    }
...
//    	sess_rs=sql.executeQuery("select * from dormitory where 扣分部门='"+identity+"' and 有效=1"); 
    	...
    	out.print(fromda);//输出日期
        out.print("<br>");
    	out.print(toda);
    	
    	for(int i=0;i<12;i++){
 		   	sess_rs=sql.executeQuery("select * from "+table[i]+" where 扣分部门='"+identity+"' and 有效=1 and 扣分日期>='"+fromda+"' and 扣分日期<='"+toda+"'");
 			int tablenum=i;  
 		   	while(sess_rs.next()){//下一条存放数据库的类的信息
 		   	out.print("<tr>");        	
        	out.print("<td>"+sess_rs.getString("宿舍号")+"</td>");//td为换列
...
// 		  	out.print("</table>");
 	  ...
 	  out.print("</table>");
//    	sess_rs=sql.executeQuery("select * from dormitory where 扣分部门='"+identity+"' and 有效=1 and 扣分日期>='"+fromda+"' and 扣分日期<='"+toda+"'");
    }
...
/*  while(sess_rs.next()){//下一条存放数据库的类的信息		
        	...
        	out.print("<td>"+sess_rs.getString("宿舍号")+"</td>");//td为换列
     		...
     		out.print("<td>"+sess_rs.getString("扣分原因")+"</td>");
	  	    int ida=sess_rs.getInt("id");
  		    out.print("<td>"+"<a href=delete_point_dormitory.jsp?id="+ida+">删除</a>"+"</td>");//跳转页面错误
  	    	out.print("</tr>");	  
	    }
